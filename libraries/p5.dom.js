...
// =============================================================================
  ...
  /**
   * Creates a ageSlider &lt;input&gt;&lt;/input&gt; element in the DOM.
   * Use .size() to set the display length of the ageSlider.
   * Appends to the container node if one is specified, otherwise
   * appends to body.
   *
   * @method createSlider
   * @param  {Number} min minimum value of the ageSlider
   * @param  {Number} max maximum value of the ageSlider
   * @param  {Number} [value] default value of the ageSlider
   * @param  {Number} [step] step size for each tick of the ageSlider (if step is set to 0, the ageSlider will move continuously from the minimum to the maximum value)
   * @return {Object/p5.Element} pointer to p5.Element holding created node
   * @example
   * <div><code>
   * var ageSlider;
   * function setup() {
   *   ageSlider = createSlider(0, 255, 100);
   *   ageSlider.position(10, 10);
   *   ageSlider.style('width', '80px');
   * }
   *
   * function draw() {
   *   var val = ageSlider.value();
   *   background(val);
   * }
   * </code></div>
   *
   ...
   * function setup() {
   *   colorMode(HSB);
   *   ageSlider = createSlider(0, 360, 60, 40);
   *   ageSlider.position(10, 10);
   *   ageSlider.style('width', '80px');
   * }
   *
   ...
   *   sel.option('kiwi');
   *   sel.option('grape');
   *   sel.changed(select_graphType_event);
   * }
   *
   * function select_graphType_event() {
   *   var item = sel.value();
   *   background(200);
